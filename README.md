...
[![状态](https://img.shields.io/badge/%E7%8A%B6%E6%80%81-%E8%83%BD%E5%8A%9B%E9%A2%84%E8%A7%88%E7%89%88%20Beta-f59e0b?style=flat-square)](#八当前状态与测试说明)
[![隐私](https://img.shields.io/badge/%E9%9A%90%E7%A7%81-%E6%97%A0%E5%BF%85%E8%A6%81%E7%95%99%E5%AD%98%E7%94%A8%E6%88%B7%E6%95%B0%E6%8D%AE-10b981?style=flat-square)](#六数据隐私与安全)

*上传一份数据文件，AI Agent 负责统计、建模与成文报告；您只负责提问与决策。*

</div>

> **关于本仓库**：DAINS 是非开源项目，本仓库不包含源代码，仅作为产品介绍门面——帮您在三分钟内了解 DAINS 的背景、能力、架构与隐私立场，并引导您完成在线端测试。

---

## 目录

- [一、DAINS 是什么](#一dains-是什么)
- [二、九大板块：一个完整的分析闭环](#二九大板块一个完整的分析闭环)
- [三、5 分钟上手（在线体验指引）](#三5-分钟上手在线体验指引)
- [四、一棵完整的分析流程树](#四一棵完整的分析流程树)
- [五、产品架构](#五产品架构)
- [六、数据隐私与安全](#六数据隐私与安全)
- [七、路线图](#七路线图)
- [八、当前状态与测试说明](#八当前状态与测试说明)
- [九、反馈与问卷](#九反馈与问卷)
- [License](#license)
...
DAINS（Data AI Insight Studio）是一个**对话式统计分析工作台**：把一份数据文件交给它，AI Agent 会完成统计探查、建模与成文报告；全过程 Python 沙箱真实执行，每一段结论都附上生成它的代码。
...
<details>
<summary><b>随处可查的功能说明（小彩蛋）</b></summary>

在任意主功能页面**按住 Alt 键（macOS 为 Option）约 2 秒**，即可显示当前阶段各要素的功能说明。
...
| 分区 | 板块 | 一句话能力 | 细节亮点 |
|---|---|---|---|
| 数据区 | **源数** | 数据文件统一管理 | 双通道上传、类型白名单、N 级目录树、焦点勾选跨板块联动 |
| 数据区 | **宽表** | 上传即透视 | 新文件默认产出统计透视报告：逐字段类型识别、缺失画像、分布特征，点状计算一目了然 |
| 产出区 | **分析** | 主菜：AI 问数 | 普通/检索式两种提交、HITL 意图确认、报告带四层支撑（结论/大纲/依据/代码）、HTML / PDF 导出 |
| 产出区 | **建模** | 两阶段 AutoML | LightGBM + Optuna 自动调参、消融特征确认卡人工圈定、Markdown 流式成稿 |
| 产出区 | **工具** | 把套路沉淀下来 | 分析逻辑提炼为 in / out 标准化工具，换数据集一键复用；内置数据质量探查等工具 |
| 产出区 | **习惯** | 您的口径，DAINS 记住 | 个人习惯（偏好）+ 统计口径（计算约束）两层沉淀 |
| 监控区 | **看板** | 从一问一答到持续浏览 | AI 一句话建板、19 种图表 + 6 种统计表、拖拽画布、只读链接免登录分享 |
| 监控区 | **调度** | 自动化动力源 | 看板定期刷新、外部数据定期抽送、临时文件定期清理 |
| 配置区 | **系统** | 账户 / 配置 / 问卷 | 产品功能计划板、问卷收集、欢迎文档与使用文档 |
...
- 当前版本为**能力预览版（Beta）**，以线上方式提供服务；产品级私有化交付（Docker Compose / Windows 桌面版）是明确的落地方向。
...
诚挚邀请您花三分钟填写问卷：登录 [dains.cn](https://dains.cn) 后，进入左侧【系统】板块的【问卷收集】。功能期待、使用障碍、建议与意见，都是产品演进的第一手依据——您的每一条反馈都会进入产品功能计划板，逐条跟进。
